...
# # label - Složka s DOCX soubory
# word_folder_entry = Label(window, text="Složka s DOCX soubory", font=("Arial", 12))
# word_folder_entry.grid(row=0, column=0, padx=10, pady=10)

# # label - Složka pro nové PDF soubory
# pdf_box = Label(window, text="Složka pro nové PDF soubory", font=("Arial", 12))
# pdf_box.grid(row=1, column=0, padx=10, pady=10)

# řádka, kam se vypíše adresa docx 
word_folder_entry = Entry(window, width=1)
#word_folder_entry.grid(row=0, column=1, padx=10, pady=10)

# řádka, kam se vypíše adresa pdf 
pdf_folder_entry = Entry(window, width=1)
#pdf_folder_entry.grid(row=1, column=1, padx=20, pady=20)


# tlačítko procházet složky uživatelem - docx
word_folder_button = Button(window, text="Procházet", font=("Arial", 12), command=browse_word_folder, image=docx_img, compound="bottom", width=170, height=240)
word_folder_button.grid(row=0, column=0, padx=20, pady=20)

# tlačítko procházet složky uživatelem - pdf 
pdf_folder_button = Button(window, text="Procházet", font=("Arial", 12), command=browse_pdf_folder, image=pdf_img, compound="bottom", width=170, height=240)
pdf_folder_button.grid(row=0, column=2, padx=20, pady=20)


# převod
convert_button = Button(window, text="Spustit převod", font=("Arial", 12), command=main, bg=button_color)
